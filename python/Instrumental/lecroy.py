...
def convert_comm_header(msg):
    comm_header = 'SHORT'
    msg = msg.replace('\x00','').strip().upper()
    try:
...
class LeCroyScope(Scope, VisaMixin):
    ...
    def get_waveform(self, channel_name, verbose = False):
        ...
        if header == 'OFF':
            ...
            print('ERROR: Wrong COMM_HEADER format {} {}'.format(header, ":".join("{:02x}".format(ord(c)) for c in header)))
            return None

        #Get the data type. For 12 bit oscilloscope use WORD only
        data_type = self.comm_format_data_type

        #ATTENTION: the feature of having WAVE_ARRAY_2 trace was not verified
        try:
            num_bytes_1 = int(self.get_waveparam(channel_name, 'WAVE_ARRAY_1', verbose=False))
        except:
            num_bytes_1 = 0

        try:
            num_bytes_2 = int(self.get_waveparam(channel_name, 'WAVE_ARRAY_2', verbose=False))
        except:
            num_bytes_2 = 0

        try:
            num_samples = int(self.get_waveparam(channel_name, 'WAVE_ARRAY_COUNT', verbose=False))
        except:
            num_samples = 0

        reply_len = header_len + num_bytes_1 + num_bytes_2

        if verbose:
            print('Reply length', reply_len)

        if self.resolution == 12 and data_type == 'BYTE':
            print('WARNING: Waveform data type: {} is less than oscilloscope resolution {}'.foramt( data_type, self.resolution) )
        ...
        #This code is taken from tektronix.py driver
        with self.resource.ignore_warning(visa.constants.VI_SUCCESS_MAX_CNT),\
            visa_context(self.resource, timeout=10000, read_termination=None,
                          end_input=visa.constants.SerialTermination.none):
            ...
            reply, status = visalib.read(session, reply_len)

            start_num_bytes = reply[:header_len].find(b'#9') + 2

            if start_num_bytes > 0:
                if verbose:
                    print(reply[start_num_bytes:start_num_bytes+9])

                num_bytes = 0

                try:                
                    num_bytes = int(reply[start_num_bytes:start_num_bytes+9])
                except:
                    print('ERROR: number of bytes in the header is wrong')
                    print('Beginning of the header'.format(reply[start_num_bytes:start_num_bytes+9]))
        ...
        if self.comm_format_data_type == 'WORD':
            num_points = num_bytes // 2
        elif self.comm_format_data_type == 'BYTE':
            num_points = num_bytes

        if num_points < num_samples:
            print('ERROR: the trace was not completely transferred.')

        raw_data_y = None

        if verbose:
            print(len(reply[start_num_bytes+10:start_num_bytes+10+num_bytes]))
            print(num_points)

        if self.comm_format_data_type == 'WORD':
            raw_data_y = np.frombuffer(reply[start_num_bytes+10:start_num_bytes+10+num_bytes], dtype='<i2', count=num_points)
        else:
            raw_data_y = np.frombuffer(reply[start_num_bytes+10:start_num_bytes+10+num_bytes], dtype='<i1', count=num_points)
...
class Waverunner625Zi(LeCroyScope):
    ...
    _INST_VISA_INFO_ = ('LeCroy', 'WR625ZI', RESOLUTION['8bits'], ANALOGUE_CHANNELS['4'])
